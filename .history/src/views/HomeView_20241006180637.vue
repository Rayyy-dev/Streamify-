<template>
  <div class="p-8 bg-gradient-to-br from-gray-900 via-gray-800 to-black min-h-screen">
    <section v-for="(section, index) in sections" :key="index" class="mb-12">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-white text-3xl font-bold">{{ section.title }}</h2>
        <button class="text-gray-400 hover:text-white transition-colors text-sm">See All</button>
      </div>

      <div class="relative">
        <button @click="scroll(index, 'left')" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full z-10">
        ...
        <button @click="scroll(index, 'right')" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full z-10">

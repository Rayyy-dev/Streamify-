...
<template>
  <div>
    <!-- Top Navigation -->
    <div id="TopNav" class="w-[calc(100%-240px)] h-[60px] fixed right-0 z-20 bg-[#101010] bg-opacity-80 flex items-center justify-between px-8">
      <div class="flex items-center space-x-4">
        <button class="bg-black bg-opacity-50 rounded-full p-1" @click="$router.go(-1)">
          <ChevronLeftIcon :size="24" fillColor="#FFFFFF" />
        </button>
        <button class="bg-black bg-opacity-50 rounded-full p-1" @click="$router.go(1)">
          <ChevronRightIcon :size="24" fillColor="#FFFFFF" />
        </button>
      </div>
      <div class="flex items-center space-x-4">
        <button class="text-white hover:text-green-500 transition-colors">Sign Up</button>
        <button class="bg-white text-black px-4 py-2 rounded-full hover:bg-opacity-80 transition-colors">Log In</button>
      </div>
    </div>
    ...
    <div id="SideNav" class="h-[100%] w-[240px] fixed z-50 bg-black flex flex-col">
      ...
      <div class="mt-6 px-2">
        <ul>
          <li>
            <button class="flex items-center w-full py-2 px-4 text-gray-300 hover:text-white transition-colors">
              <PlusIcon :size="20" />

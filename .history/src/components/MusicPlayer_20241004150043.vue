    ...
    <div id="MusicPlayer" v-if="audio" class="
        ...
        <div class="flex items-center w-1/4">
            <div class="flex items-center ml-4">
                <img class="rounded-md shadow-xl" width="50" :src="currentArtist.albumCover">
                <div class="ml-4">
        ...
        <div class="max-w-[40%] mx-auto w-2/4 mb-1">
            <div class="flex-col items-center justify-center">
                <div class="buttons flex items-center justify-center h-[30px]">
                    <button class="mx-3 bg-gray-800 p-2 rounded-full hover:bg-gray-700">
                        <SkipBackward fillColor="#FFFFFF" :size="20" @click="useSong.prevSong(currentTrack)" />
                    </button>
                    <button class="p-3 rounded-full mx-4 bg-white hover:bg-gray-200"
                ...
                <div class="flex items-center h-[20px] mt-2">
                    <div v-if="isTrackTimeCurrent" class="text-gray-400 text-[11px] pr-2">{{ isTrackTimeCurrent }}</div>
                    <div ref="seekerContainer" class="w-full relative" @mouseenter="isHover = true"
                        ...
                        <div class="pointer-events-none absolute h-[3px] z-10 inset-y-0 left-0 w-0"
                            :style="`width: ${range}%;`" :class="isHover ? 'bg-white' : 'bg-gray-500'" />
                        <div
                            class="absolute h-[3px] z-[-0] inset-y-0 left-0 w-full bg-gray-700 rounded-full" />
                    </div>
                    <div v-if="isTrackTimeTotal" class="text-gray-400 text-[11px] pl-2">{{ isTrackTimeTotal }}</div>
                </div>
            </div>
        </div>
